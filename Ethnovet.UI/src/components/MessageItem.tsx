  ...
  const formatContent = (content: string) => {
    ...
    return lines.map((line, idx) => {
      ...
      if (trimmed.startsWith('### ')) {
        return (
          <h3 key={idx} className="text-sm font-bold text-emerald-950 mt-3 mb-1 first:mt-0">
            {trimmed.replace('### ', '')}
          </h3>
        );
      }
      if (trimmed.startsWith('## ')) {
        return (
          <h2 key={idx} className="text-base font-bold text-emerald-950 mt-3 mb-1 first:mt-0">
            {trimmed.replace('## ', '')}
          </h2>
        );
      }
      ...
      if (trimmed.includes('⚠️') || trimmed.toLowerCase().includes('veterinarian') || trimmed.includes('கால்நடை மருத்துவர்')) {
        return (
          <div
            key={idx}
            className="my-2 p-2.5 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2 text-xs text-amber-900 leading-relaxed font-medium"
          >
    ...
    <div className={`flex items-start gap-3 my-4 ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      <div className={`max-w-[85%] sm:max-w-[75%] space-y-2.5 ${isUser ? 'items-end text-right' : 'items-start text-left'}`}>
        ...
        >
          ...
          {!isUser && (
            <div className="flex items-center justify-between border-b border-slate-100 pb-1.5 mb-2 text-[11px] text-slate-400">
              ...
              <button
                onClick={handleCopy}
                className="flex items-center gap-1 hover:text-slate-600 transition-colors p-0.5 rounded-sm"
                title="Copy response"
              >
        ...
        {!isUser && message.relevantRemedies && message.relevantRemedies.length > 0 && (
          <div className="space-y-2 pt-1">
            <div className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider px-1">
      ...
      {/* Avatar (User) */}
      {isUser && (
        <div className="w-8 h-8 rounded-xl bg-slate-700 text-white flex items-center justify-center shadow-xs shrink-0 mt-0.5">
